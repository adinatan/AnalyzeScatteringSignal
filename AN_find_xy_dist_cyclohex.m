...
global im10k2M x2M y2M  Polarization Geometry smask s q
...
%  imagesc(im)
%  
tm=permute(reshape(testmask(:),size(im10k2M)),[3 2 1] );

binratio=0.1;
dm=AN_get2Ddata2(x2M(:),y2M(:),tm(:) ,binratio);

% find center:
%%
close all
binratio=100;
im=AN_get2Ddata2(x2M(:),y2M(:),im10k2M(:).*smask(:),binratio);
im(im==0)=NaN;
...
Cy=interp1(1:numel(nx),nx,mean(c(:,1)),'spline')
Cx=interp1(1:numel(ny),ny,mean(c(:,2)),'spline')
...
clear init_vals
options = optimset('MaxFunEvals',200,'MaxIter',200,'TolX',1e-4,'TolFun',1e-4,'Display' , 'iter');
%options = optimset( 'Display' , 'iter');
tic 
% init_vals=[init_det_dist init_x init_y init_scale];
init_vals=[44e3 Cx Cy 2e3 1.5e5];

LB = [ 43e3 Cx-10 Cy-10  1e3 1e5];
UB = [ 45e3 Cx+10 Cy+10 3e3 2e5 ];

[x,fval,exitflag] = fminsearchbnd(@opt_dist_cyclohexmodel ,init_vals,LB,UB,options);
toc

%% check what we got:
%x=[44e3 Cx Cy  2e3 1.5e5];
detdist = x(1);% in mm
...
St=interp1(q(~isnan(s)) ,s(~isnan(s)),xq,'spline');
%
figure
plot(xq,St.*x(end-1)+x(end),'LineWidth',1); hold on
plot(xq,b0a,'LineWidth',1);
xlim([0 7.72])
