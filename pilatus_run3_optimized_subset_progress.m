...
% ------------------------------------------------------------------------
% FILE SUBSET TO PROCESS
%
% These refer to positions in the numerically sorted TIFF list.
%
% Examples:
%   FILE_START = 1;     FILE_END = 1000;   % first 1000 TIFFs
%   FILE_START = 1001;  FILE_END = 5000;   % TIFFs 1001 through 5000
%   FILE_START = 1;     FILE_END = inf;    % complete acquisition
%
% IMPORTANT:
% Pairing is fixed as:
%   file 2 - file 1, file 4 - file 3, ...
%
% Therefore FILE_START must be ODD so the selected subset starts on the
% first member of an acquisition pair. FILE_END may be any value; any
% incomplete final WINDOW is ignored automatically.
% ------------------------------------------------------------------------
FILE_START = 1;
FILE_END   = inf;
...
% ========================================================================
% APPLY USER-SELECTED FILE SUBSET
% ========================================================================

if ~isscalar(FILE_START) || FILE_START < 1 || FILE_START ~= floor(FILE_START)
    error('FILE_START must be a positive integer.');
end
...
if FILE_END > nFilesTotal
    error('FILE_END=%d exceeds the %d TIFFs available.',FILE_END,nFilesTotal);
end

% Preserve the global acquisition pairing:
% (1,2), (3,4), (5,6), ...
if mod(FILE_START,2) == 0
    error(['FILE_START=%d is even. Start on an odd file position so pairing ' ...
           'remains file 2-file 1, file 4-file 3, ...'],FILE_START);
end

selectionIdx = FILE_START:FILE_END;

paths     = paths(selectionIdx);
names     = names(selectionIdx);
fileBytes = fileBytes(selectionIdx);
frameIDs  = frameIDs(selectionIdx);
